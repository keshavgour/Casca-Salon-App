    ...
    { dataTODisplay.map((data) => (
       ...
       <Grid container spacing={4} justifyContent='space-between'>
           <Grid item mb={2} ml={2}>
              <Grid container>
                <Grid item >
                <Avatar src={data.imageURL}  alt='salonImages' sx={{width:150, height:150, borderRadius:'20%'}} ></Avatar>
                </Grid>
                <Grid item>
                <Grid container direction={'column'} spacing={3} pl={2}>
                    <Grid item>
                        <Typography variant='h5' sx={{fontWeight:'bold'}}>{data.shopName}</Typography>
                    </Grid>
                    <Grid item>
